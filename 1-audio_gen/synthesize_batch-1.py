...
# --- Project Name (Hardcoded for now, as per your request) ---
# This will be replaced by app-wide config later
PROJECT_NAME = "coach-dashboard" # Using a generic project name for this batch run

# Ensure the specific project output directory exists
PROJECT_AUDIO_OUTPUT_DIR = BASE_AUDIO_OUTPUT_DIR / PROJECT_NAME

# Text chunk limit for OpenAI TTS (as per your existing code)
CHUNK_LIMIT = 3500

# Average speaking rate for estimation (150 words per minute = 2.5 words per second)
...
def synthesize_batch_scripts():
    """
    Synthesizes audio for all script files in the _selected_scripts directory
    after user verification, reporting progress and time taken.
    """
    ...
    # List available script files for user verification
    ...
    proceed = input("\nDoes the list look good to proceed? (y/n): ").lower().strip()
    if proceed != 'y':
        print("Batch synthesis aborted by user.")
        return

    # Prepare the project-specific output directory
    ...
    for i, script_path in enumerate(script_files):
        ...
        try:
            ...
            for j, chunk in enumerate(chunks):
                ...
                # print(f"    Synthesizing chunk {j+1}/{len(chunks)}...") # Optional: detailed chunk progress
            ...
            # Determine output audio filename: script_basename.mp3
            # E.g., n8n_hosting_script_1.0.txt -> n8n_hosting_script_1.0.mp3
            ...
            # Consider adding more robust error logging here later
